...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function unlock(){
    ...
    if(!pibo_db){
      ...
      const warning = `<p style="color:var(--orange-dark);font-weight:700">
        اتصال به پایگاه‌داده برقرار نیست — مقادیر js/firebase-config.js را طبق README تکمیل کنید.
      </p>`;
      if(grid) grid.innerHTML = warning;
      if(body) body.innerHTML = `<tr><td colspan="4">${warning}</td></tr>`;
      return;
    }
...
async function initProductManager(){
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try{
      ...
      alert("ذخیره پیتزا با خطا مواجه شد. اتصال اینترنت و تنظیمات Firebase را بررسی کنید.");
      console.error(err);
    }finally{
...
function renderProductTable(products){
  ...
  if(!products.length){
    body.innerHTML = `<tr><td colspan="4" style="text-align:center;color:var(--ink-soft)">هنوز پیتزایی اضافه نشده است.</td></tr>`;
    return;
  }
