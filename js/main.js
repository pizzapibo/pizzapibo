...
async function renderMenu(){
  ...
  if(!products.length){
    grid.innerHTML = `<p style="color:var(--ink-soft);grid-column:1/-1;text-align:center">
      هنوز پیتزایی در منو ثبت نشده است. از پنل مدیریت پیتزا اضافه کنید.
    </p>`;
    return;
  }
...
function fillPickListAndQr(){
  ...
  if(qrHolder && PIBO_PRODUCTS.length){
    const first = PIBO_PRODUCTS[0];
    const url = location.href.replace(/index\.html.*$/, "").replace(/\/?$/, "/") + `ar.html?pizza=${first.id}`;
